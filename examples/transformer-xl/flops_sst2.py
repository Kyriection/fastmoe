...
parser.add_argument('--dataset', type=str, default='wt103',
                    choices=['wt103', 'lm1b', 'enwik8', 'text8', 'csqa', 'sst2'],
                    help='dataset name')
...
if not args.pretrained_weight == 'random':
...
def evaluate(model, eval_iter):
    ...
    with torch.no_grad():
        ...
        for i, (data, mask, label) in enumerate(eval_iter):
            ...
            macs, params = profile(model, inputs=(data, mask, *mems), custom_ops={FMoELinear: count_flinear})
...
def train():
    ...
    for batch, (data, mask, label) in enumerate(train_iter):
        ...
        predict, mems = para_model(data, mask, mems)
        macs, params = profile(para_model, inputs=(data, mask, *mems), custom_ops={FMoELinear: count_flinear})
        print('Train {}, {:E}, {:E}, {:E}'.format(data.shape, 2*macs, 2 * macs * 5000, params))
