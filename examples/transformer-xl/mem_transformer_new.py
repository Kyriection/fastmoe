...
from proj_adaptive_softmax import ProjectedAdaptiveLogSoftmax_new, Projection
...
class MemTransformerLM(nn.Module):
    def __init__(self, n_token, n_layer, n_head, d_model, d_head, d_inner,
        ...
        # use adaptive softmax (including standard softmax)
            ...
            self.crit = ProjectedAdaptiveLogSoftmax_new(n_token, d_embed, d_model,
    ...
    def forward(self, data, target, *mems):
        ...
        if self.sample_softmax > 0 and self.training:
            ...
            loss, _ = self.crit(pred_hid.view(-1, pred_hid.size(-1)), target.contiguous().view(-1))
            loss = loss.view(tgt_len, -1)

        if new_mems is None:
            return [pred_hid, loss]
        else:
            return [pred_hid, loss] + new_mems
